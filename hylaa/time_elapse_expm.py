...
class TimeElapseExpmMult(Freezable):
    ...
    def __init__(self, time_elapser, discrete):
    ...
    def assign_basis_matrix(self, step_num):
        ...
        if step_num == 0: # step zero, basis matrix is identity matrix
            ...
            if self.b_csc is not None:
                ...
                # prev_input_effects_matrix = self.cur_input_effects_matrix
                self.cur_input_effects_matrix = np.dot(self.one_step_matrix_exp, self.cur_input_effects_matrix)
                # print(self.cur_input_effects_matrix)
                # total_input_effects_matrix_wo_input = prev_input_effects_matrix + self.cur_input_effects_matrix
                # total_input_effects_matrix_wo_input = total_input_effects_matrix_wo_input + self.b_csc.todense()
                # print(total_input_effects_matrix_wo_input)
                #
                # input_val = np.array([[0.39340481], [-0.03933961]], dtype=float)
                # total_input_effects_matrix_w_input = np.dot(total_input_effects_matrix_wo_input, input_val)
                # print(total_input_effects_matrix_w_input)
